...
class Unet(MaskingModel):
    ...
    def __init__(self):
        ...
        if bundle_dir:
            weight_path = os.path.join(bundle_dir, 'nancy_weights.h5')
            model_path = os.path.join(bundle_dir, 'nancy_model.json')
        else:
            weight_path = '/src/nancy_weights.h5'
            model_path = '/src/nancy_model.json'
